...
Pipefy::Database -- A writeable representation of a database in Pipefy

=head1 SYNOPSIS

 my $client = Pipefy::Client->new({ api_key => $pipefy_oauth_token }); 
 my $database = $client->databases();

=head1 DESCRIPTION

This is the class returned by Pipefy::Client when you execute methods that return user objects. It is created from a blob of JSON taken from the Pipefy API, and so goes stale immediately.
